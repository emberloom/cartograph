...
pub fn structural_signal(
    ...
    match min_depth {
        Some(depth) if depth > 0 => 1.0 / (depth as f64),
        _ => 0.0,
    }
}
    ...
    for changed_file in changed_files {
        ...
        for result in &co {
            if result.entity_path.as_deref() == Some(candidate) {
                let c = if result.confidence.is_finite() {
                    result.confidence
                } else {
                    0.0
                };
...
pub fn ownership_signal(store: &GraphStore, candidate: &str) -> f64 {
    ...
    if owner_count <= 1 {
        ...
        let count_f = owner_count as f64;
        let normalized = (count_f - 1.0) / count_f;
        normalized.min(1.0)
    }
}
...
mod tests {
    ...
    fn test_structural_signal_no_connection() {
        ...
        // The function should not panic regardless of result
        assert!(signal >= 0.0);
    }
    ...
    #[test]
    fn test_hotspot_signal_unknown_file() {
        let store = setup_store();
        let signal = hotspot_signal(&store, "nonexistent.rs");
        assert_eq!(signal, 0.0, "unknown file should return 0");
    }
