...
pub fn parse_typescript_source(source: &str, path: &Path) -> ParseResult {
    ...
    if parser.set_language(&language).is_err() {
        ...
        return ParseResult { entities: Vec::new(), imports: Vec::new(), modules: Vec::new() };
    }

    let Some(tree) = parser.parse(source, None) else {
        tracing::warn!("tree-sitter failed to parse TypeScript source ({} bytes)", source.len());
        return ParseResult { entities: Vec::new(), imports: Vec::new(), modules: Vec::new() };
    };

    let mut result = ParseResult { entities: Vec::new(), imports: Vec::new(), modules: Vec::new() };
...
mod tests {
    ...
    fn ts_path() -> &'static Path { Path::new("src/foo.ts") }
    fn tsx_path() -> &'static Path { Path::new("src/foo.tsx") }
