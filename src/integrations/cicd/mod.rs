...
/// Format CiReport as pretty-printed JSON.
pub fn format_json_pretty(report: &CiReport) -> Result<String, serde_json::Error> {
...
mod tests {
    ...
    #[test]
    fn test_fail_threshold_exit_codes_for_each_level() {
        // FailThreshold::None never fails
        assert!(!FailThreshold::None.should_fail(&RiskLevel::Low));
        assert!(!FailThreshold::None.should_fail(&RiskLevel::Medium));
        assert!(!FailThreshold::None.should_fail(&RiskLevel::High));
        assert!(!FailThreshold::None.should_fail(&RiskLevel::Critical));

        // FailThreshold::Low fails on everything
        assert!(FailThreshold::Low.should_fail(&RiskLevel::Low));
        assert!(FailThreshold::Low.should_fail(&RiskLevel::Medium));
        assert!(FailThreshold::Low.should_fail(&RiskLevel::High));
        assert!(FailThreshold::Low.should_fail(&RiskLevel::Critical));

        // FailThreshold::Medium fails on medium+
        assert!(!FailThreshold::Medium.should_fail(&RiskLevel::Low));
        assert!(FailThreshold::Medium.should_fail(&RiskLevel::Medium));
        assert!(FailThreshold::Medium.should_fail(&RiskLevel::High));
        assert!(FailThreshold::Medium.should_fail(&RiskLevel::Critical));

        // FailThreshold::High fails on high+
        assert!(!FailThreshold::High.should_fail(&RiskLevel::Low));
        assert!(!FailThreshold::High.should_fail(&RiskLevel::Medium));
        assert!(FailThreshold::High.should_fail(&RiskLevel::High));
        assert!(FailThreshold::High.should_fail(&RiskLevel::Critical));

        // FailThreshold::Critical only fails on critical
        assert!(!FailThreshold::Critical.should_fail(&RiskLevel::Low));
        assert!(!FailThreshold::Critical.should_fail(&RiskLevel::Medium));
        assert!(!FailThreshold::Critical.should_fail(&RiskLevel::High));
        assert!(FailThreshold::Critical.should_fail(&RiskLevel::Critical));
    }
    ...
    fn test_format_json_pretty() {
        ...
        let json = format_json_pretty(&report).unwrap();
