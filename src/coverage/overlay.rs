...
mod tests {
    use super::*;
    use crate::coverage::store::write_coverage;
    use crate::store::graph::GraphStore;
    use crate::store::schema::{EdgeKind, EntityKind};

    fn setup() -> (GraphStore, Connection) {
        let conn = Connection::open_in_memory().unwrap();
        crate::store::schema::init_db(&conn).unwrap();
        crate::coverage::store::init_coverage_table(&conn).unwrap();

        let mut store = GraphStore::new(
            Connection::open_in_memory()
                .unwrap()
                .tap(|c| crate::store::schema::init_db(c).unwrap()),
        )
        .unwrap();
    ...
    // Helper trait for inline initialization
    trait Tap: Sized {
    ...
    impl Tap for Connection {}

    #[test]
    fn test_find_coverage_gaps() {
        let (store, conn) = setup();
        let gaps = find_coverage_gaps(&store, &conn, 2, 10).unwrap();
